...
describe('boolean', () => {
	...
	it('must be', () => {
		const schema = boolean().mustBe(true).compile();
	
		expect(schema.isValid(true)).toBeTruthy()
		expect(schema.isValid(false)).toBeFalsy()

		const schema1 = boolean().mustBe(false).compile();
	
		expect(schema1.isValid(true)).toBeFalsy()
		expect(schema1.isValid(false)).toBeTruthy()
	})

...
import { type ExcludeByValueType } from '../types/types';
...
export type AnyJsonSchema = ArraySchema<any, any> | BooleanSchema<any, any> | DateSchema<any, any> | NumberSchema<any, any> | ObjectSchema | StringSchema<any, any>;
...
export type JsonSchemaType = JsonSchemaArrayType 
	...
	if ( schemaValue.validation ) {
		if ( schemaValue.validation.required ) {
			schema = schema.required(
				typeof schemaValue.validation.required === 'string'
					? schemaValue.validation.required
					: undefined
			);
		}
...
function validation<T extends AnyJsonSchema>(
	...
	.forEach((key) => {
		...
		schema = (schema as any)[key as unknown as ExcludeByValueType<NonNullable<JsonSchemaStringValidationType>, boolean | string>](
			typeof schemaValidation === 'string'
				? schemaValidation
				: undefined
		);
	});

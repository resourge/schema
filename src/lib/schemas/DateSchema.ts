...
> extends Schema<Input, Final> {
	...
	public equals<Form = this['final']>(
		...
		const equalsDateValue = typeof date === 'string'
			? new Date(date)
			: date;
		...
		return this.test({
			...
			name: equalsDateValue instanceof Date
				? `equals_${equalsDateValue.toISOString()}_${format}_${message}`
				: undefined
		});
	}
	...
	public maxDate<Form = this['final']>(
		...
		const maxDateValue = typeof maxDate === 'string'
			? new Date(maxDate)
			: maxDate;
		...
		return this.test({
			...
			name: maxDateValue instanceof Date
				? `maxDate_${maxDateValue.toISOString()}_${format}_${message}`
				: undefined
		});
	}
	...
	public minDate<Form = this['final']>(
		...
		const minDateValue = typeof minDate === 'string'
			? new Date(minDate)
			: minDate;
		return this.test({
			is: this.getComparisonFunction(
			...
			name: minDateValue instanceof Date
				? `minDate_${minDateValue.toISOString()}_${format}_${message}`
				: undefined
		});
	}
	...
	public today(message?: string) {
		return this.test({
			is: (value: Date | string) => !isToday(typeof value === 'string'
				? new Date(value)
				: value),
	...
	private getComparisonFunction<Form = this['final']>(
		...
		return (value: Date | string, parent: any, config: ValidationContext<Form>) => {
			return _cb(
				getDate(typeof date === 'string'
					? new Date(date)
					: date, parent, config), 
				extractTime(typeof value === 'string'
					? new Date(value)
					: value)
			);
		};
